...
def expanded_linear_layer(in_dim, out_dim, expansion_factor):
...
class ModelFromPaper(nn.Module):
    def __init__(self, expansion_factor):
        ...
        self.linear1 = expanded_linear_layer(512, 256, expansion_factor)
        self.linear1_activation = nn.ReLU()
        self.linear2 = nn.Sequential(expanded_linear_layer(256, 10, expansion_factor))
...
class ModelFromPaperWithLowRankLayer(nn.Module):
    def __init__(self, rank1):
        super().__init__()
        self.cnn1 = nn.Sequential(
            nn.Conv2d(3, 64, 3, 1, 1), 
            nn.MaxPool2d(2),
        ...
        self.linear1 = LowRankLinear(512, 256, rank1)
        self.linear1_activation = nn.ReLU()
        self.linear2 = expanded_linear_layer(256, 10, 2)
        self.linear2_activation = nn.ReLU()
